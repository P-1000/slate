...
function createWindow() {
  ...
  // Send message to Renderer after window is loaded
...
function registerHotkey() {
  globalShortcut.register('Cmd+Shift+Space', () => {
    if (win) {
      ...
      if (win.isVisible()) {
        win.hide()
        console.log("Window hidden from hotkey")
      } else {
        // If the window is not visible, show it
        win.show()
        console.log("Window shown from hotkey")
      }
    } else {
...
app.on('activate', () => {
  if (BrowserWindow.getAllWindows().length === 0) {
    createWindow() // Recreate window if all windows are closed
  }
})

// Wait for the app to be ready before creating window and tray
app.whenReady().then(() => {
  createWindow()  
  createTray()
  registerHotkey()
...
app.on('window-all-closed', () => {
  if (process.platform !== 'darwin') {
    app.quit() // Quit the app if no windows open
  }
})
